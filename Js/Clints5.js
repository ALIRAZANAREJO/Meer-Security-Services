...
const resContainer = document.getElementById('resContainer5');
...
function displayData(id, { Cnic, Name, Dob, Fname, Phone, Caddres, Paddres, Qual, ImageUrl ,}) {
 ...
 <div class="input-container">
        ...
        <div class="input-wrapper">
            <input type="text" id="Fname2" placeholder=" " value="${Paddres}" required>
            <label for="Fname2">Email</label>
        </div>

        <div class="input-wrapper">
            <input type="text" id="Cnic" placeholder=" " value="${Cnic}" required>
...
<select style="display: none;" id="containerSelect6">
    <option style="display: none;" value="resContainer6">resContainer6</option>
...
window.acceptRequest = function (id) {
    ...
    const dbRef = ref(db, `resContainer5/${id}`);
    ...
    get(dbRef)
        .then((snapshot) => {
            if (snapshot.exists()) {
                ...
                if (!clientData.Paddres) {
                    alert("❌ No email found for this client!");
                    return;
                }

                // Send confirmation email
                sendEmailToClient(clientData.Paddres, clientData.Name);
...
function sendEmailToClient(clientPaddres, clientName) {
    fetch("https://formsubmit.co/ajax/" + clientPaddres, {
        method: "POST",
        headers: {
