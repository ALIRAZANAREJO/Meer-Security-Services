...
const resContainer7 = document.getElementById('resContainer7');
...
submitBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        const newCnicRef = ref(db, `resContainer7/${cnic.value}`);
...
recallBtn.addEventListener('click', async () => {
    ...
    try {
        const snapshot = await get(child(ref(db), `resContainer7/${cnicValue}`));
        if (snapshot.exists()) {
...
function fetchAndDisplayCards() {
    ...
    onValue(dbRef, (snapshot) => {
        resContainer7.innerHTML = ""; // Clear existing cards
...
function createCard(id, { Cnic, Name, Dob, Fname, Phone, Caddres, Paddres, Qual, ImageUrl, PdfUrl,Lisnt,Rarmy,Citybox }) {
    ...
    resContainer7.appendChild(card); // Append to the results container
...
updateBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        await update(ref(db, `resContainer7/${cnicValue}`), updatedData);
...
deleteBtn.addEventListener('click', async () => {
    ...
    try {
        await remove(ref(db, `resContainer7/${cnicValue}`));
