...
const resContainer10 = document.getElementById('resContainer10');
...
submitBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        const newCnicRef = ref(db, `resContainer10/${cnic.value}`);
...
recallBtn.addEventListener('click', async () => {
    ...
    try {
        const snapshot = await get(child(ref(db), `resContainer10/${cnicValue}`));
        if (snapshot.exists()) {
...
function fetchAndDisplayCards() {
    const dbRef = ref(db, "resContainer10");
    onValue(dbRef, (snapshot) => {
        resContainer10.innerHTML = ""; // Clear existing cards
...
function createCard(id, { Cnic, Name, Dob, Fname, Phone, Caddres, Paddres, Qual, ImageUrl, PdfUrl,Lisnt,Rarmy,Citybox }) {
    ...
    resContainer10.appendChild(card); // Append to the results container
...
updateBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        await update(ref(db, `resContainer10/${cnicValue}`), updatedData);
...
deleteBtn.addEventListener('click', async () => {
    ...
    try {
        await remove(ref(db, `resContainer10/${cnicValue}`));
...
//   const pdfInput = document.getElementById('pdfUploadd');
//   const fileNameDisplayed = document.getElementById('pdfFileNameDisplayed');

//   pdfInput.addEventListener('change', function () {
//     if (this.files && this.files.length > 0) {
//       fileNameDisplayed.textContent = `Selected File: ${this.files[0].name}`;
//     } else {
//       fileNameDisplayed.textContent = '';
//     }
//   });
