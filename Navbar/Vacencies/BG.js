...
const resContainer6 = document.getElementById('resContainer6');
...
submitBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        const newCnicRef = ref(db, `resContainer6/${cnic.value}`);
...
recallBtn.addEventListener('click', async () => {
    ...
    try {
        const snapshot = await get(child(ref(db), `resContainer6/${cnicValue}`));
        if (snapshot.exists()) {
...
function fetchAndDisplayCards() {
    ...
    onValue(dbRef, (snapshot) => {
        resContainer6.innerHTML = ""; // Clear existing cards
...
function createCard(id, { Cnic, Name, Dob, Fname, Phone, Caddres, Paddres, Qual, ImageUrl }) {
    ...
    resContainer6.appendChild(card); // Append to the results container
...
updateBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        await update(ref(db, `resContainer6/${cnicValue}`), updatedData);
...
deleteBtn.addEventListener('click', async () => {
    ...
    try {
        await remove(ref(db, `resContainer6/${cnicValue}`));
